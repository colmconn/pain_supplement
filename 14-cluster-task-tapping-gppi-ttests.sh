...
if [[ $overwrite -eq 1 ]] || [[ ! -f $csvFile ]] ; then 
    echo "count,seedName,tLabelPrefix,contrastBrikId,statsBrikId,tThreshold,NN,nVoxels,pValue,alpha,nClusters,tTestFile" > $csvFile
fi

((seedCount=1 ))
for seed in ${seeds} ; do
    sc_pp=$( printf "%02d" ${seedCount} )
    seedName=$( cat ${TASK_TTEST_PIPELINE_DIR}/clusters.locations.${tLabelPrefix}.csv | \
		    awk -F"," -v rr=${seedCount} '{print $rr}' | \
		    tr -d '()' )
    
    msg=$( printf "Clustering t-tests for seed %s : %s" ${sc_pp} ${seedName} )
    info_message_ln "${msg}"
    cd ${PIPELINE_DIR}

    dataTableFilename=datatable_count-${sc_pp}_seed-${seedName}.tsv
    info_message_ln "Data table file is: $dataTableFilename"

    tTestFile=count-${sc_pp}_seed-${seedName}_${ttests_task}+tlrc
    ...
    csimfile=count-${sc_pp}_seed-${seedName}_${ttests_task}.CSimA.NN${NN}_${side}sided.1D
    ...
    suffix="count-${sc_pp}_seed-${seedName}_${tLabelPrefix}"
    ...
    echo "${sc_pp},${seedName},${tLabelPrefix},${contrastBrikId},${statsBrikId},${tThreshold},${NN},${nVoxels},${pValue},${alpha},${nClusters},${tTestFile}" >> ${csvFile}
