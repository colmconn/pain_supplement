...
if [[ -f ${session_norm} ]] && [[ ${base_norm} ]]  ; then
    info_message_ln "Found subject ${subjectNumber} ${session} session and base norm.mgz"
...
recon-all -long ${ss} ${subjectNumber} -all
...
EOF
	...
	LOG_FILE=${LOG_DIR}/${ss}_freesurfer_long.log
	info_message_ln "To see progress run: tail -f $LOG_FILE"
	rm -f ${LOG_FILE}

	qsub -N ${ss}_freesurfer_long -q ${q} -pe ${pe} ${threads} -j y -m n -V \
