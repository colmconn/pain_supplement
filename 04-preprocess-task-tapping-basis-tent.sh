...
declare -a epiFiles
if (( ${nechos} == 1 )) ; then
    sourceEpiFiles[0]=$( cd ${unprocessedDataDir}; ls -1 ${ss}_task-tapping_run-?_bold.nii.gz | sort -t '_' -k 4.5n )
else
    for ee in $( seq 1 ${nechos} ) ; do
	sourceEpiFiles[${ee}]=$( cd ${unprocessedDataDir}; ls -1 ${ss}_task-tapping_run-?_echo-${ee}_bold.nii.gz | sort -t '_' -k 4.5n -k 5.6n )
    done
fi
...
threeDDeconPrefix=basis-tent
outputScriptName=${CODE_DIR}/run/${ss}_preprocess-${task}_${threeDDeconPrefix}.sh
...
for (( ee=1; ee <= ${nechos}; ee++ )) ; do
    ...
    info_message_ln "${msg}"
    if [[ ${ee} -eq 1 ]] ; then 
	dsets_me_echo_arg+="-dsets_me_echo ${epiFiles[$ee]} "$'\\'$'\n'
    else
	if [[ ${ee} -eq ${nechos} ]] ; then
	    dsets_me_echo_arg+="             -dsets_me_echo ${epiFiles[$ee]} "
	else
	    dsets_me_echo_arg+="             -dsets_me_echo ${epiFiles[$ee]} "$'\\'$'\n'	    
	fi
...
preprocessingScriptPrefix=${ss}_preprocess-${task}-${scriptExt}
preprocessingScriptSuffix=".csh"
rm -f \${preprocessingScriptPrefix}\${preprocessingScriptSuffix}
...
afni_proc.py -subj_id ${ss}										\\
             -script \${preprocessingScriptPrefix}\${preprocessingScriptSuffix}.new			\\
	     -out_dir \${outputDir}									\\
	     ...
	     -regress_3dD_stop										\\
	     -regress_stim_times  ${regressorDir}/task-tapping-tr-locked.1D				\\
	     -regress_stim_labels tapping								\\
	     -regress_basis 'TENT(0,32,21)'								\\
	     ...
	     -write_3dD_prefix ${threeDDeconPrefix}_							\\
	     -write_3dD_script \${preprocessingScriptPrefix}_${threeDDeconPrefix}\${preprocessingScriptSuffix}	\\
	     -html_review_style pythonic

## the next will only be true if afni proc completed correctly, then will execute the script using tcsh language
if [[ -f \${preprocessingScriptPrefix}_${threeDDeconPrefix}\${preprocessingScriptSuffix} ]] ; then 

   cp ${regressorDir}/task-tapping-tr-locked.1D \${outputDir}/stimuli
   ( cd \${outputDir} ; tcsh -xef ../\${preprocessingScriptPrefix}_${threeDDeconPrefix}\${preprocessingScriptSuffix}; tcsh -x ${threeDDeconPrefix}_stats.REML_cmd -GOFORIT )
    ...
    echo "*** No such file \${preprocessingScriptPrefix}_${threeDDeconPrefix}\${preprocessingScriptSuffix}"
