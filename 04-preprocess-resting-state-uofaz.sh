...
declare -a epiFiles
sourceEpiFiles=( $( ls -1 ${unprocessedDataDir}/${ss}_task-rest_bold.nii.gz | sort -t '_' -k 4.5n ) )
...
afni_proc.py -subj_id ${ss}									\\
	     ...
	     -out_dir \${outputDir}									\\
	     -blocks despike tshift align tlrc volreg mask ${blurBlock} scale regress		       	\\
	     -copy_anat $anatFile									\\
