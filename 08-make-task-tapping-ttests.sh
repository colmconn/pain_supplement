...
first_stats_file=$( head -1 baseline_stats_files.txt  )
...
3dttest++ \
    -setA $( cat baseline_stats_files.txt ) \
    -mask Bmask_epi+tlrc.HEAD \
    -prefix baseline \
    -Clustsim

3dttest++ \
    -setA $( cat followup_stats_files.txt ) \
    -mask Bmask_epi+tlrc.HEAD \
    -prefix followup \
    -Clustsim

3dttest++ -setA  $( cat baseline_stats_files.txt ) \
	  -setB  $( cat followup_stats_files.txt ) \
	  -labelA baseline \
	  -labelB followup \
	  -paired \
	  -mask Bmask_epi+tlrc.HEAD \
	  -prefix baseline-followup \
	  -Clustsim
